const { TipBook, assertTipAmount, TIP_CREATOR_BPS, TIP_HOUSE_BPS } = require("../src/tips");
const { makeWallet } = require("../src/wallet");

const eq = (a, b, m) => { if (Math.abs(a - b) > 1e-6) throw new Error(m + `: ${a} != ${b}`); };
function assert(cond, msg) { if (!cond) throw new Error(msg); }

assert(TIP_CREATOR_BPS === 10_000 && TIP_HOUSE_BPS === 0, "tips 100/0 creator");
...
try { assertTipAmount(NaN); throw new Error("nan should throw"); }
catch (e) { if (!/Tip must be between/.test(e.message)) throw e; }
...
assert(row.creatorBps === 10_000 && row.houseBps === 0, "recorded split");
assert(row.toCredits === false && row.toPrize === false, "tips never fund credits or prizes");
assert(row.toPot === false && row.toSeat === false, "tips never fund pots or seats");
...
(async () => {
  ...
  const dest = { address: creator };
  const tx = await w.ante(spectator, dest, 2);
  eq(await w.getBalance(creator), 2, "100% of tip credited to creator address");
