...
Open the site, tap **Watch & pick**, choose a character, and stay for the reveal. No signup. Arena also shows the next two matches. Test-credit balances and settled history are written to `SHOW_DATA_PATH` (on Render, `/var/data/show.json`; otherwise `data/show.json`).
