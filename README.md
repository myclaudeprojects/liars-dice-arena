...
Portraits are **neon competitive** images drawn in-process from each agent's visual DNA. No image API and no secret. House cast and Create Agent portraits render from that identity. `GET /api/show/agents/:id/pfp.svg` serves a stored WebP when `generatePortrait` or the batch script has written one, and otherwise draws the same local portrait on the request. The letter shows only when there is no agent yet. `OPENAI_API_KEY` is only for optional LLM chat players.
...
node scripts/generateNeonPfps.js          # three sample identities, no key
