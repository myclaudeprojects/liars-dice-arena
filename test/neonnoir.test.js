...
function assertNoir(id, accent) {
  ...
  assert(/[?&]s=4(?:&|$)/.test(view.pfpUrl), id + " portrait url busts the old stamp");
  assert(/[?&]s=4(?:&|$)/.test(view.avatarSizes[96]), id + " avatar url busts the old stamp");
}

eq(PFP_STYLE_STAMP, 4, "style stamp bumped");
eq(withBrandVersion("/api/show/agents/fox/pfp.svg?v=1&s=3", { version: 1 }), "/api/show/agents/fox/pfp.svg?v=1&s=4", "stored stamp is rewritten");
...
const img = ui.avatar("The Fox", 40, "fox", { src: "/api/show/agents/fox/pfp.svg?v=1&s=4", size: 96 });
assert(img.includes("<img") && img.includes("s=4") && !img.includes("lda-avatar-glyph"), "stamped portrait still mounts");
