...
class Table {
  ...
  async oneMatch() {
    ...
    this.pool = new BettingPool({ wallet: this.mgr.wallet, houseFeeBps: HOUSE_FEE_BPS, potLabel: labels.pool });
    await this.pool.init();
    this.phase = "betting";
    this.bets = [];
    this.multipliers = impliedMultipliers([], agents.map((a) => a.id), HOUSE_FEE_BPS);
    ...
    const settlement = await this.pool.settle(result.winnerId || null, this.mgr.houseWallet);
  ...
  async buildAgents(table) {
    ...
    for (const a of this.registry.list()) {
      ...
      try {
        ...
        if ((await this.wallet.getBalance(w.walletId)) >= this.ante) funded.add(a.id);
    ...
    for (const rec of recs) {
      ...
      if (rec.house) {
        ...
        if (this.wallet.ensureFunded) {
          try {
            const tx = await this.wallet.ensureFunded(ag.walletInfo, this.ante);
...
module.exports = {
  Table, TableManager, Mutex, TABLE_ID_RE, HOUSE_FEE_BPS,
